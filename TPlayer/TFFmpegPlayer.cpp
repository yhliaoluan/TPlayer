...
int TFFmpegPlayer::Convert(FF_VIDEO_FRAME *in, FF_FRAME *out)
{
	int ret = FF_OK;
...
void TFFmpegPlayer::ThreadStart()
{
	...
	FF_FRAME frame = {0};
	FF_VIDEO_FRAME *videoFrame = NULL;
	FF_VIDEO_FRAME vf = {0};
	while (1)
	{
		if (holdMutex)
...
void TFFmpegPlayer::SyncVideo(FF_FRAME *f)
{
...
int TFFmpegPlayer::GetMediaInfo(FF_SETTINGS *pSettings)
{
	if (_ctx->videoStream)
	{
...
int TFFmpegPlayer::SetAudioOutputSetting(FF_AUDIO_SETTING *setting)
{
	if (_audioDecoder)
...
int TFFmpegPlayer::Init(const FF_INIT_SETTING *setting)
{
	int ret;
	ret = InitCtx(setting);
...
int TFFmpegPlayer::InitCtx(const FF_INIT_SETTING *setting)
{
	int ret = 0;
	char szFile[1024] = {0};
	...
	FreeCtx();
	_ctx = (FF_CONTEXT *)av_mallocz(sizeof(FF_CONTEXT));
	_ctx->vsIndex = -1;
	_ctx->asIndex = -1;
...
void TFFmpegPlayer::OnNewFrame(FF_FRAME *p)
{
	if (_fNewFrameCB)
		_fNewFrameCB(p);
}
